...
class Test05ReviewAPI:
    def test_01_review_not_auth(
        ...
        response = client.get(f'/api/v1/titles/{titles[0]["id"]}/review/')
        assert response.status_code != HTTPStatus.NOT_FOUND, (
            'Эндпоинт `/api/v1/titles/{title_id}/review/` не найден, '
            'проверьте настройки в *urls.py*.'
        )
        assert response.status_code == HTTPStatus.OK, (
            'Проверьте, что GET-запрос неавторизованного пользователя к '
            '`/api/v1/titles/{title_id}/review/` возвращает ответ со '
            'статусом 200.'
        )

        response = client.post(
            f'/api/v1/titles/{titles[0]["id"]}/review/', data=new_data
        )
        ...
        response = client.patch(
            f'/api/v1/titles/{titles[0]["id"]}/review/{reviews[1]["id"]}/',
            data=new_data,
        )
        assert response.status_code == HTTPStatus.UNAUTHORIZED, (
            'Проверьте, что PATCH-запрос неавторизованного пользователя к '
            '`/api/v1/titles/{title_id}/review/{review_id}/` возвращает '
    ...
    def test_02_review_post(
        ...
        response = user_client.post(
            f'/api/v1/titles/{titles[0]["id"]}/review/', data=data
        )
        assert response.status_code == HTTPStatus.BAD_REQUEST, (
            'Если POST-запрос авторизованного пользователя к '
            '`/api/v1/titles/{title_id}/review/` содержит некорректные '
        ...
        assert response.status_code == HTTPStatus.BAD_REQUEST, (
            ...
            '`/api/v1/titles/{title_id}/review/` вернёт ответ со '
            'статусом 400.'
        )
        ...
        try:
            ...
            assert False, (
                'Не удалось импортировать модели из приложения review. '
                f'Ошибка: {e}'
            )
        ...
        assert type(response.json().get('id')) == int, (
            ...
            '`/api/v1/titles/{title_id}/review/` возвращает данные '
            'созданного объекта. Сейчас поля `id` нет в ответе или его '
            'значение не является целым числом.'
        )

        data = {'text': 'На один раз', 'score': 4}
        response = user_client.post('/api/v1/titles/999/review/', data=data)
        assert response.status_code == HTTPStatus.NOT_FOUND, (
            'Проверьте, что POST-запрос авторизованного пользователя к '
            '`/api/v1/titles/{title_id}/review/` для несуществующего '
            'произведения возвращает ответ со статусом 404.'
        )

        data = {'text': 'Супер!', 'score': 11}
        response = user_client.post(
            f'/api/v1/titles/{titles[1]["id"]}/review/', data=data
        )
        assert response.status_code == HTTPStatus.BAD_REQUEST, (
            'Если в POST-запросе авторизованного пользователя к '
            '`/api/v1/titles/{title_id}/review/` передана оценка выше 10 '
        ...
        assert response.status_code == HTTPStatus.BAD_REQUEST, (
            'Если в POST-запросе авторизованного пользователя к '
            '`/api/v1/titles/{title_id}/review/` передана оценка ниже 1 '
            'балла - должен вернуться ответ со статусом 400.'
        )

        url = f'/api/v1/titles/{titles[0]["id"]}/review/'
        response = user_client.get(f'/api/v1/titles/{titles[0]["id"]}/review/')
        assert response.status_code == HTTPStatus.OK, (
        ...
        assert review, (
            ...
            '`/api/v1/titles/{title_id}/review/` возвращается вся информация '
    ...
    def test_03_review_detail_get(
        ...
        reviews, titles = create_reviews(admin_client, author_map)
        url_template = '/api/v1/titles/{title_id}/review/{review_id}/'

        response = client.get(
            f'/api/v1/titles/{titles[0]["id"]}/review/{reviews[0]["id"]}/'
        )
        assert response.status_code != HTTPStatus.NOT_FOUND, (
    ...
    def test_04_review_detail_user(
        ...
        response = user_client.patch(
            f'/api/v1/titles/{titles[0]["id"]}/review/{reviews[2]["id"]}/',
            data=new_data,
        )
